...
impl ModuleHost {
    ...
    pub fn call_views_with_tx_at<I: WasmInstance>(
        ...
        for view_call in tx.views_for_refresh().cloned().collect::<Vec<_>>() {
            ...
            let Some((fn_ptr, view_def, owning_def)) =
                module_def.view_by_name_with_global_fn_ptr(view_def.name.as_raw().as_ref())
            else {
                outcome = ViewOutcome::Failed(format!("view for view id `{}` not found in module", view_id));
                break;
            };
